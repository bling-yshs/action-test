	...
	<div class="center-preview">
		...
		<div class="center-info">
			...
			<div class="info-item3" v-if="userTableName=='yonghu'">
				...
				<div class="text">{{sessionForm.chepaihao}}</div>
			</div>
		...
		<el-tabs class="center-tabs" tab-position="left" @tab-click="handleClick">
			<el-tab-pane label="个人中心">
				<el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="180px">
					...
					<el-form-item class="center-item" v-if="userTableName=='yonghu'" label="车牌号" prop="chepaihao">
...
<script>
	...
	export default {
		...
		created() {
			...
			if ('yonghu' == this.userTableName) {
				this.$set(this.sessionForm, 'chepaihao', null);
			}
			if ('yonghu' == this.userTableName) {
			...
			if ('yonghu' == this.userTableName&&this.rules['chepaihao']){
				this.rules['chepaihao'].push({ required: true, message: '请输入车牌号', trigger: 'blur' })
			}else if('yonghu' == this.userTableName&&!this.rules['chepaihao']) {
				this.$set(this.rules, 'chepaihao', [{ required: true, message: '请输入车牌号', trigger: 'blur' }]);
			}
			if ('yonghu' == this.userTableName) {
