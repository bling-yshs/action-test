	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/feiyongxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">停车时长</div>
						<div class="text "  >{{detail.tingcheshizhang}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">停车费用</div>
						<div class="text "  >{{detail.tingchefeiyong}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">费用备注</div>
						<div class="text "  >{{detail.feiyongbeizhu}}</div>
					</div>
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('feiyongxinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('feiyongxinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="payBtn" v-if="centerType&&(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('feiyongxinxi','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.detailBanner = this.detail.fapiao ? this.detail.fapiao.split(",") : [];
						this.$forceUpdate();
			...
			payClick(){
				localStorage.setItem('paytable','feiyongxinxi')
			...
			backClick(){
				...
				this.$router.push({path: '/index/feiyongxinxi', query: params});
			},
			// 下载
			download(file ){
				if(!file) {
			...
			// 修改
			editClick(){
				this.$router.push(`/index/feiyongxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此费用信息？') .then(_ => {
					this.$http.post('feiyongxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
