	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/cheweixinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">车位状态</div>
						<div class="text "  >{{detail.cheweizhuangtai}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">车位区域</div>
						<div class="text "  >{{detail.cheweiquyu}}</div>
					</div>
					<div class="item">
						<div class="lable">车位位置</div>
						<div class="text "  >{{detail.cheweiweizhi}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('cheweixinxi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('cheweixinxi','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('cheweixinxi','停车')" 
							@click="onAcross('ruchangxinxi','','','cheweizhuangtai','已停车','已停车,闲置中'.split(',')[0])" type="warning">
							停车
						</el-button>
					</div>
				</div>
			...
			<div class="detail2">
				<div class="desc-view">
					<div class="desc-label">车位详情</div>
				</div>
				<div class="desc-text ql-snow ql-editor" v-html="detail.cheweixiangqing"></div>
			</div>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('cheweixinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '收藏成功!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'cheweixinxi', userid: localStorage.getItem('frontUserid')}}).then(res => {
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此车位信息？') .then(_ => {
